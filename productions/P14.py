...
def P14(graph: StandardizedGraph, v0_group: list, v1_group: list):
    ...
    for x in v0_vertices+v1_vertices:
        print( x.underlying, x.pos_x(), x.pos_y(), x.label())
...
def match_P14(graph: StandardizedGraph, level:int, i_small=True):
    return match(graph, level, i_small)


def match(graph: StandardizedGraph, level:int, i_small):
    matching_vertices = find_all_matching_vertices(graph, level)
    # print("matching coords: ", matching_vertices)
     
    return find_matching_groups(graph, matching_vertices, i_small)
        
def find_matching_groups(graph, matching_vertices, i_small):
    merge_parts = []
        
    for matching_vertices_combination in combinations(matching_vertices, 2):
        merge_parts.extend(find_matching_groups_for_combination(graph, matching_vertices_combination, i_small))
        
    return merge_parts # change to return i_verts
    
def find_matching_groups_for_combination(graph, matching_vertices_combination, i_small):
                                ...
                                if k != i:
                                    ...
                                    for l in range(len(second_group)):
                                        if l != j:
                                            ...
                                            if check_production_predicats(graph, vert1, vert2, vert3, vert4, vert, i_small):
...
def check_production_predicats(graph, vert1, vert2, vert3, vert4, between_vert, i_small):
    ...
    subgraph_nodes = i_0_list+ [ i_1]  + [vert1.underlying, vert2.underlying, vert3.underlying, vert4.underlying, between_vert] #+ [list(i_0_neigh.intersection(i_1_neigh))[0]]

    i_0_neigh = set(graph.get_neighbours(Vert(graph.underlying, i_0), Vert(graph.underlying, i_0).level()+1, 'I'))

    i_0_neigh = set(graph.get_neighbours(Vert(graph.underlying, i_0_list[1]), Vert(graph.underlying, i_0).level() + 1, 'I') + list(i_0_neigh))
