...
def P6(graph: StandardizedGraph, v: Vert):
    level = v.level()

    graph.modify_label(v, "i")

    e1_left, e2_left, e3_left, e4_left = get_surrounding_e_coordinates(
        graph, v)

    e1 = graph.add_vert(pos_x=e1_left.pos_x(),
                        pos_y=e1_left.pos_y(), level=level + 1, label="E")
    e2 = graph.add_vert(pos_x=e2_left.pos_x(),
                        pos_y=e2_left.pos_y(), level=level + 1, label="E")
    e3 = graph.add_vert(pos_x=e3_left.pos_x(),
                        pos_y=e3_left.pos_y(), level=level + 1, label="E")
    e4 = graph.add_vert(pos_x=e4_left.pos_x(),
                        pos_y=e4_left.pos_y(), level=level + 1, label="E")
...
def match_P6(graph: StandardizedGraph, level: int):
    matcher = SurroundingMatcher(4)
