...
class SurroundingMatcher():
    ...
    def get_graph_template(self):
        ...
        expected_subgraph.add_nodes_from([
            (0, {"label": "I"}),
            (1, {"label": "E"}),
            (2, {"label": "E"}),
            (3, {"label": "E"}),
            (4, {"label": "E"}),
            (5, {"label": "E"}), # Vertex inside edge
            (6, {"label": "E"})
        ])
    ...
    def is_breaking_point(self, graph, e1, e2, breaking_point):
        return nx.get_node_attributes(graph, "label")[breaking_point] == "E" \
            and (nx.get_node_attributes(graph, "pos_x")[e1] + nx.get_node_attributes(graph, "pos_x")[e2]) / 2 == nx.get_node_attributes(graph, "pos_x")[breaking_point] \
            and (nx.get_node_attributes(graph, "pos_y")[e1] + nx.get_node_attributes(graph, "pos_y")[e2]) / 2 == nx.get_node_attributes(graph, "pos_y")[breaking_point] 
